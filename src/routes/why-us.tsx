...
import { Video, Maximize2, X, ChevronLeft, ChevronRight, MessageCircle } from "lucide-react";
       ...
       {activeModalIndex !== null && currentItem && (
         ...
         >
           ...
           >
             ...
             <button
               onClick={handlePrev}
               className="absolute left-0 sm:-left-6 top-1/2 -translate-y-1/2 z-20 p-3 rounded-full bg-black/60 hover:bg-black/80 text-white border border-white/20 transition-all cursor-pointer hover:scale-110 shadow-2xl backdrop-blur-md"
             ...
             <button
               onClick={handleNext}
               className="absolute right-0 sm:-right-6 top-1/2 -translate-y-1/2 z-20 p-3 rounded-full bg-black/60 hover:bg-black/80 text-white border border-white/20 transition-all cursor-pointer hover:scale-110 shadow-2xl backdrop-blur-md"
    ...
    <div className={`flex flex-col lg:flex-row gap-10 lg:gap-16 items-center animate-fade-up ${isReversed ? 'lg:flex-row-reverse' : ''}`}>
       <div className="flex-1 w-full">
         <div 
           ...
           className="glass-dark rounded-[2.5rem] p-3 shadow-2xl relative group overflow-hidden border border-gold/20 bg-white/40 backdrop-blur-md cursor-pointer hover:border-gold/60 transition-all duration-300 hover:shadow-gold/20"
           title="Click to view full size"
         >
           <div className="rounded-[2rem] overflow-hidden aspect-[4/3] sm:aspect-video relative bg-emerald-deep/5 flex items-center justify-center">
             {media?.type === "image" ? (
               <img 
                 src={media.src} 
                 alt={imageAlt} 
               ...
               <div className="w-full h-full flex flex-col items-center justify-center gap-4 text-emerald-deep/30 relative z-10">
                 ...
                 <span className="font-bold text-lg sm:text-xl tracking-widest uppercase opacity-80">Media Coming Soon</span>
               </div>
             )}
             ...
             <div className="absolute bottom-4 right-4 bg-black/70 hover:bg-black/90 text-white backdrop-blur-md px-3 py-1.5 rounded-full text-xs font-semibold flex items-center gap-1.5 border border-white/20 shadow-lg opacity-80 group-hover:opacity-100 transition-opacity">
               ...
               <span className="hidden sm:inline">{media?.type === "video" ? "Watch Video" : "Click to Full View"}</span>
             </div>
           </div>
         </div>
       </div>
